...
#preform = If motors begin in preformed pairs
  ...
  totaltime = 150.0f0  #Duration of simulation  (seconds)
...
@inbounds for t in iters   #Begin simulation
    ...
    @inbounds for j in 1:numpairs #Calculate net forces on filaments due to motor pair springs
     ...
     @inbounds for j in 1:nf #Apply all forces to filaments
      ...
      if abs(filcyp[j]) > boundlen
        filcyp[j] = sign(filcyp[j])*boundlen*2.0f0 - filcyp[j]
      end
    ...
    #Apply forces on motors
     @inbounds for j in 1:nm
      ...
      if skc==skiptime
        ...
        #   if minrange(mxp[j],myp[j],actreg[1,:],[1,2],moptype[j],t,0.0f0)
...
res = t > acttime && sqrt(mxp^2+myp^2)<=actreg && (mtype == mtypes[1] || mtype == mtypes[2])
return res
end

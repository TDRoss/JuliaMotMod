...
function MotorModelv03MoveRec(argvec)
...
#preform = If motors begin in preformed pairs
  ...
  totaltime = 600.0f0  #Duration of simulation  (seconds)
            ...
            checkyp = myp[paironeID]
             ...
             @inbounds for p in 1:nf#check if filament is a potential partner
              if sfx[p,1]<checkxp<sfx[p,2] && sfy[p,1]<checkyp<sfy[p,2] && boundone != p #Check if filament is in range and exclude motor pairs binding to same filament
                t1,t2=circleintline(checkxp,checkyp,filsxp[p],filsyp[p],flen,filexp[p],fileyp[p],motln)
...
if t>stp
    res = (mxp>-40000 && mxp < 15000 && myp<7000 && myp>-7000) || (sqrt((mxp-15000)^2+myp^2)<=actreg[1])
else
    res = sqrt((mxp-15000)^2+myp^2)<=actreg[1]
end
return res
end

...
#preform = If motors begin in preformed pairs
  ...
  totaltime = 1000.0f0  #Duration of simulation  (seconds)
  ...
  maxymotloc = 25000.0f0
  maxyfilloc = 15000.0f0
  ...
  filcyp = rand(Float32,nf)*filybound -filybound/2 #y coordinate of filament center
  ...
  myp = rand(Float32,nm)*motybound - motybound/2 #y coordinate of motor
  ...
  timesaveresolution = .5f0 #time resolution of output array (seconds)
    ...
    @inbounds for j in 1:numpairs #Calculate net forces on filaments due to motor pair springs
      if mpairs[j,3] >0 && mpairs[j,4] > 0 #If motor pairs are both filament bound
        ...
        if l > motln
          ...
          forceonfils[filoneID,1] += linforce1ll
          forceonfils[filtwoID,1] += linforce2ll
          forceonfils[filoneID,2] += linforce1o
          forceonfils[filtwoID,2] += linforce2o
          forceonfils[filoneID,3] += mdfc[paironeID] * linforce1o
          forceonfils[filtwoID,3] += mdfc[pairtwoID] * linforce2o
        end
      end
    end
     ...
     @inbounds for j in 1:nf #Apply all forces to filaments
      ...
      if abs(filcxp[j]) > boundlen
        filcxp[j] = sign(filcxp[j])*boundlen*2.0f0 - filcxp[j]
      end
      if abs(filcyp[j]) > maxyfilloc
        filcyp[j] = sign(filcyp[j])*maxyfilloc*2.0f0 - filcyp[j]
      end
     ...
     @inbounds for j in 1:nm
      ...
      if skc==skiptime
        @inbounds for j in 1:nm
          if mpairID[j] == 0 #single motors
            if mbfID[j] == 0 #not bound to filament
              ...
              if abs(myp[j]) > maxymotloc
                myp[j] = sign(myp[j])*maxymotloc*2.0f0 - myp[j]
              end
            end
          else  #paired motors
            if mpairs[mpairID[j],1] == j && mpairs[mpairID[j],3] == 0 && mpairs[mpairID[j],4] == 0 #both motors not bound to filament
              ...
              if abs(myp[j]) > maxymotloc
                mpdist = 2.0f0*(sign(myp[j])*maxymotloc - myp[j])
                myp[j] += mpdist
        ...
        #   #Check if motor is in active region
          if minrange(mxp[j],myp[j],[15000.0f0],[1,2],moptype[j],t,0.0f0,stp)
		...
		bindrng = 100.0f0
             ...
             @inbounds for p in 1:nf#check if filament is a potential partner
              if sfx[p,1]<checkxp<sfx[p,2] && sfy[p,1]<checkyp<sfy[p,2] && boundone != p #Check if filament is in range and exclude motor pairs binding to same filament
                t1,t2=circleintline(checkxp,checkyp,filsxp[p],filsyp[p],flen,filexp[p],fileyp[p],bindrng)
...
function minrange(mxp::Float32,myp::Float32,actreg::Vector{Float32},mtypes::Vector{Int},mtype::Int,t::Float32,acttime::Float32,stp::Float32)
# res = t > acttime && (actreg[1]<mxp<actreg[2]) && (actreg[3]<myp<actreg[4]) && (mtype == mtypes[1] || mtype == mtypes[2])
if t>15
    res = sqrt((mxp-15000+(t-15)*stp)^2+myp^2)<=actreg[1] && (mtype == mtypes[1] || mtype == mtypes[2])
else
    res = sqrt((mxp-15000)^2+myp^2)<=actreg[1] && (mtype == mtypes[1] || mtype == mtypes[2])
end
return res
end

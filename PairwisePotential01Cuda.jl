...
function pairwisepotential(argvec)
    ...
    @inbounds for t in iters   #Begin simulation
        ...
        diffx = broadcast(-,filx,transpose(filx))
        diffy = broadcast(-,fily,transpose(fily))
        ...
        vx = dspeed.*sum(inzn.*CUDAnative.cos.(theta),1)[1,:]./normy
        vy = dspeed.*sum(inzn.*CUDAnative.sin.(theta),1)[1,:]./normy
        ...
        if tc == indxstep #check if it is time to save state
          ...
          # end
          potdist = pairwise(Euclidean(),filhist[hc,:,collect(vindx)],mgrid[:,ptinzn])
          potdist[potdist.>flen] = 0
          pothist[hc,ptinzn] = dspeed.*mean(potdist,1)
          # potdist = filx
...
function isinzone(filx,fily,t)
    ...
    #     if t<250
    #     indx = (sqrt.((filx-25000.0f0).^2+(fily+25000.0f0).^2) .<= 10000.0f0) .| (sqrt.((filx+25000.0f0).^2+(fily+25000.0f0).^2) .<= 10000.0f0) .|
    #     (sqrt.((filx).^2+(fily-17000.0f0).^2) .<= 10000.0f0)
    # elseif t<1400
    #         indx =    (sqrt.((filx).^2+(fily-17000.0f0).^2) .<= 10000.0f0) .|
    #         ((-27000.0f0.<filx.<27000.0f0) .& (-27500.0f0.<fily.<-22500.0f0))
    #     else
    #         # indx = (sqrt.((filx).^2+(fily-15000.0f0).^2) .<= 10000.0f0) .|
    #         indx = (-30000.0f0.<fily.<19000.0f0) .& (-2500.0f0.<filx.<2500.0f0)

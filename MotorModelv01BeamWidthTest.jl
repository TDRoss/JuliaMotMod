...
function MotorModelv01BeamWidthTest(argvec)
...
vw=argvec[7]::Float64
vn=argvec[8]::Float64
flen=argvec[9]::Float64
motln=argvec[10]::Float64
boundlen=argvec[11]::Float64
koff=argvec[12]::Float64
kon=argvec[13]::Float64
intensity=argvec[14]::Float64
beamh=argvec[15]::Float64
...
#preform = If motors begin in preformed pairs
  ...
  #Initialize costants
  dt = 0.1*10^(-4.); #time step size (seconds)
  skiptime = Int(floor(5*10^(-2.)/dt)) #time between collision detection events
  totaltime = 500.0  #Duration of simulation  (seconds)
  ...
  timesaveresolution = 2 #time resolution of output array (seconds)
  ...
  actreg = [-10*flen 10*flen -beamh/2.0 beamh/2.0]
      ...
      if skc==skiptime
        @inbounds for j in 1:nm
          ...
          if (actreg[1]<mxp[j]<actreg[2] && actreg[3]<myp[j]<actreg[4])
            if rand()<intensity #probability of motor being activated is proportional to light intensity
